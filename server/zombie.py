...
class Zombie(Entity):
    ...
    def takeTurn(self, board):
        ...
        for h in humans:
            if not h.isDead() and h._square!=None and (board.checkLOS(self._square, h._square) or board.isAdjacent(self._square, h._square)):
        ...
        if closestHuman != None and int(shortestDistance) <= self._agroRange:
            ...
            if abs(dy) <=1 and abs(dx) <=1:
                self.attack(closestHuman._square)
            else:
